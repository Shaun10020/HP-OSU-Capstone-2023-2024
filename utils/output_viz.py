...
def print_output(idx,masks, outputs):
    ...
    for i in range(len(labels)):
        ...
        axs[1, i].imshow(convertBinary(masks[idx][i]).cpu().detach().numpy(), cmap='gray')
        axs[1, i].axis('off')  # Hide axes for clarity
        # Display each output in the second row
        axs[2, i].imshow(outputs[idx][i].cpu().detach().numpy(), cmap='gray')
        axs[2, i].axis('off')  # Hide axes for clarity
